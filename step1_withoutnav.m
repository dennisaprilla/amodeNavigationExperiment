...
% for the file savings
is_saving = true;
LABEL_1   = "withoutnav-raw";
LABEL_2   = "withoutnav-cleaned";
filelabel = LABEL_2;

% declare some of important names
% declare some of important names
if(strcmp(filelabel, LABEL_1))
    groups = [ "A_F_GTR", ...
               "A_F_MID", ...
               "A_F_LEP", ...
               "A_F_MEP", ...
...
all_measurements = struct('groupname', "", 'probes', [], 'rigidbodies', []);
...
for group_idx=1:n_groups
    ...
    rigidbody = struct('number', 0, 'timestamp', 0, 'tforms', []);
    ...
    for rbdata_idx=1:n_observation
        ...
        T_ref_global       = current_grouprigidbody(rbdata_idx).Probes(1).T;
        % [!] Unfortunately, in without-navigation experiment, we fail to
        %     detect B_N_REF for two regions. The T value become NaN. To
        %     prevent errors, we use the average B_N_REF of other region,
        %     assuming B_N_REF is not moving.
        isNan_T_ref_global = any(isnan(T_ref_global), 'all');
        if(isNan_T_ref_global)
        ...
        for k=1:length(signal_ticks_idx)-1
            signal_lowres(k) = mean(signal(signal_ticks_idx(k):signal_ticks_idx(k+1)));
        end
        
        % 4.2) get the signal sample that is inside the window
        ...
        % ---- this one is for outside the window
        ...
        % ---- this one is for inside the window
        ...
        % ---- this one is for 3d signal peak (from observation in situ)
        ...
        % ---- this one is for 3d signal peak (from post processing)
        ...
        % 5.1) plot 3d signal outside the window
        ...
        % 5.2) plot 3d signal inside the window
        ...
        % 5.4) plot the text label for each signal
        ...
        % 5.5) plot the text label for each signal
        text( ax1{group_idx},...
        ...
        rigidbody(rbdata_idx).number    = rbdata_idx;
        rigidbody(rbdata_idx).timestamp = current_grouprigidbody(rbdata_idx).Timestamp;
        % [!] Unfortunately, in without-navigation experiment, we fail to
        %     detect B_N_REF for two regions. The T value become NaN. To
        %     prevent errors, we use the average B_N_REF of other region,
        %     assuming B_N_REF is not moving.
        if(isNan_T_ref_global)
            tmp   = current_grouprigidbody(rbdata_idx).Probes(1);
            tmp.T = T_ref_global;
            rigidbody(rbdata_idx).tforms    = [current_grouprigidbody(rbdata_idx).Stylus; tmp; current_grouprigidbody(rbdata_idx).Probes(2:end)];
        else
            rigidbody(rbdata_idx).tforms    = [current_grouprigidbody(rbdata_idx).Stylus; current_grouprigidbody(rbdata_idx).Probes];
        end
    ...
    all_measurements(group_idx).rigidbodies = rigidbody;
...
if(is_saving)
    ...
    file_output = ['output-measurement_'+filelabel+'_'+currentTime+'.mat'];
